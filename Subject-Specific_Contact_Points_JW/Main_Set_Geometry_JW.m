...
Segment(3).Generic.T_implant2bone = [...
    0.9947,    0.0212,   -0.1002,   -0.0029; ...
   -0.0200,    0.9997,    0.0134,   -0.0070; ...
    0.1004,   -0.0114,    0.9949,    0.0000; ...
         0,         0,         0,    1.0000];
% Virtual palpation in CT/CAD model (femur)
Segment(5).Generic.T_implant2bone = [...
     0.9937,   -0.0474,   -0.1015,   -0.0029; ...
     0.0415,    0.9974,   -0.0593,   -0.0070; ...
     0.1040,    0.0548,    0.9931,    0.0000; ...
          0          0          0     1.0000]; % Origin at distal endpoint
Segment(5).Generic.T_implant2bone = [eye(3), ...
    Segment(5).Generic.T_implant2bone(1:3,1:3)*[0; 0.4002;0]; ... % From proximal to distal endpoint in implant coordinate system
...
Segment(5).Generic.rVs(:,6) = inv(Segment(5).Generic.T_implant2bone)*...
    [([-0.0079; 0.0093; -0.0146] + ... % Medial sphere of femur throchlea
    [-0.0075; 0.0048; 0.0240])/2;1]; % Lateral sphere of femur throchlea
Segment(5).Generic.rVs(4,:) = [];

% Axis orientation
Segment(5).Generic.ns(:,1) = inv(Segment(5).Generic.T_implant2bone(1:3,1:3))*...
    ([-0.0075; 0.0048; 0.0240] - [-0.0079; 0.0093; -0.0146])/...
    norm([-0.0075; 0.0048; 0.0240] - [-0.0079; 0.0093; -0.0146]);

% Hinge in patella
Segment(4).Generic.rVs(:,1) = inv(Segment(5).Generic.T_implant2bone(1:3,1:3))*...
        (([-0.0079; 0.0093; -0.0146] + ... % Medial sphere of femur throchlea
    [-0.0075; 0.0048; 0.0240])/2 -... % Lateral sphere of femur throchlea
    [43; -5; 6]/1000); % Origin of patella (estimated by Fregly for the CAD of the full leg)
...
Segment(3).Generic.rVs(1:4,9) = inv(Segment(3).Generic.T_implant2bone)*...
    [0.0359;-0.0942;0.0072;1]; % Patellar tendon insertion
...
Tfimplant2timplant = [eye(3),-[-0.0029; 0.0426; 0.0003]; [0,0,0,1]];
%
[Triangles,Points] = stlread('Tibia_Fibula_And_Tibial_Insert.stl');
Points(:,1) = - Points(:,1); % Mesh definition
Points(:,3) = - Points(:,3); % Mesh definition
Segment(3).Generic.Vertices = (inv(Segment(3).Generic.T_implant2bone)*Tfimplant2timplant*...
    [Points';ones(1,length(Points))])';
...
Segment(3).Generic.L = 0.4220; % From JW CT-scan
Segment(3).Generic.W = 0.0728; % From JW CT-scan
Segment(4).Generic.L = norm([45.22; -24.53; 0.05]/1000 - ... % Patellar tendon origin
    [39.0; -0.36; -2.79]/1000); % Origin of patella;
Segment(5).Generic.L = 0.4002; % From JW CT-scan
Segment(5).Generic.W = 0.0861; % From JW CT-scan
%
Joint(4).Generic.d(1,1) = norm([0.0533;-0.0288;0.0105] - ... % Patellar tendon origin (CAD of patella)
    [0.0359;-0.0942;0.0072]); % Patellar tendon insertion (CAD of tibia)
