...
Segment(3).Generic.T_implant2bone = [...
    0.9889,   -0.0162,   -0.1476,   -0.0043; ...
    0.0227,    0.9988,    0.0424,    0.0007; ...
    0.1467,   -0.0453,    0.9881,    0.0007; ...
         0,         0,         0,    1.0000];
% Virtual palpation in CT/CAD model (femur)
Segment(5).Generic.T_implant2bone = [...
    0.9853,   -0.0872,   -0.1466,   -0.0043; ... 
    0.0869,    0.9962,   -0.0084,    0.0007; ... 
    0.1467,   -0.0045,    0.9892,    0.0007; ...
         0         0         0    1.0000]; % Origin at distal endpoint
Segment(5).Generic.T_implant2bone = [eye(3), ...
    Segment(5).Generic.T_implant2bone(1:3,1:3)*[0; 0.4152;0]; ... % From proximal to distal endpoint in implant coordinate system
...
Segment(5).Generic.rVs(:,6) = inv(Segment(5).Generic.T_implant2bone)*...
    [([-5.36114; 2.184453; 23.896042]/1000 + ... % Medial sphere of femur throchlea
    [-6.105056; 4.71; -23.59]/1000)/2;1]; % Lateral sphere of femur throchlea
Segment(5).Generic.rVs(4,:) = [];

% Axis orientation
Segment(5).Generic.ns(:,1) = inv(Segment(5).Generic.T_implant2bone(1:3,1:3))*...
    ([-5.36114; 2.184453; 23.896042] - [-6.105056; 4.71; -23.59])/...
    norm([-5.36114; 2.184453; 23.896042] - [-6.105056; 4.71; -23.59]);

% Hinge in patella
Segment(4).Generic.rVs(:,1) = inv(Segment(5).Generic.T_implant2bone(1:3,1:3))*...
        (([-5.36114; 2.184453; 23.896042]/1000 + ... % Medial sphere of femur throchlea
    [-6.105056; 4.71; -23.59]/1000)/2 - ... % Lateral sphere of femur throchlea
...
Segment(3).Generic.rVs(1:4,9) = inv(Segment(3).Generic.T_implant2bone)*...
    [[35.6; -90.56; 0.88]/1000;1]; % Patellar tendon insertion
...
Tfimplant2timplant = [eye(3),-[6.5155; 41.3000; 0]/1000; [0,0,0,1]];
%
...
Segment(3).Generic.L = 0.4262; % From DM CT-scan
Segment(3).Generic.W = 0.0690; % From DM CT-scan
Segment(4).Generic.L = norm([45.22; -24.53; 0.05]/1000 - ... % Patellar tendon origin
    [39.0; -0.36; -2.79]/1000); % Origin of patella;
Segment(5).Generic.L = 0.4152; % From DM CT-scan
Segment(5).Generic.W = 0.0887; % From DM CT-scan
%
Joint(4).Generic.d(1,1) = 66.733/1000;
