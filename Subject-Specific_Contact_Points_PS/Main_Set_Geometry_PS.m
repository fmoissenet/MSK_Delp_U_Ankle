...
Segment(5).Generic.T_implant2bone = [eye(3), ...
    Segment(5).Generic.T_implant2bone(1:3,1:3)*[0; 0.4256;0]; ... % From proximal to distal endpoint in implant coordinate system
...
Segment(5).Generic.rVs(:,6) = inv(Segment(5).Generic.T_implant2bone)*...
    [([-4.585; 4.718; 24.008*(-1)]/1000 + ...  % Medial sphere of femur throchlea (left knee)
    [-5.52; 3.136; -24.34*(-1)]/1000)/2;1]; % Lateral sphere of femur throchlea (left knee)
Segment(5).Generic.rVs(4,:) = [];

% Axis orientation
Segment(5).Generic.ns(:,1) = inv(Segment(5).Generic.T_implant2bone(1:3,1:3))*...
    ([-5.52; 3.136; -24.34*(-1)] - [-4.585; 4.718; 24.008*(-1)])/...
    norm([-5.52; 3.136; -24.34*(-1)] - [-4.585; 4.718; 24.008*(-1)]);

% Hinge in patella
Segment(4).Generic.rVs(:,1) = inv(Segment(5).Generic.T_implant2bone(1:3,1:3))*...
        (([-4.585; 4.718; 24.008*(-1)]/1000 + ...  % Medial sphere of femur throchlea (left knee)
    [-5.52; 3.136; -24.34*(-1)]/1000)/2 - ...  % Lateral sphere of femur throchlea (left knee)
    [42.1; -4.2; -2.72*(-1)]/1000); % Origin of patella (left knee)
...
Segment(3).Generic.L = 0.4316; % From PS CT-scan
Segment(3).Generic.W = 0.0695; % From PS CT-scan
Segment(4).Generic.L = norm([45.22; -24.53; 0.05]/1000 - ... % Patellar tendon origin
    [39.0; -0.36; -2.79]/1000); % Origin of patella;
Segment(5).Generic.L = 0.4256; % From PS CT-scan
Segment(5).Generic.W = 0.0871; % From PS CT-scan
%
Joint(4).Generic.d(1,1) = 55.127/1000;
