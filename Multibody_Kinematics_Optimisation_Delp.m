...
function [Segment,Joint] = Multibody_Kinematics_Optimisation_Delp(Segment,Joint,f)
...
ns12 = [0.781;0.600;-0.120]/norm([0.781;0.600;-0.120]);
% Subtalar axis position in foot segment
rVs12 = (-[0.0005; -0.3994; 0.0075] + ... % From knee joint centre to mid-malleolus
    [0; -0.4260; 0]); % ... % From knee joint centre to talus centre
%     *(Segment(3).L/0.3914);  % Scaled by shank length (0.3914 from mid-epicondyle to mid-malleolus)
% No rVs22
% No rVs32

% Ankle axis direction in shank segment
ns13 = [-0.105; -0.174; 0.979]/norm([-0.105; -0.174; 0.979]);
% Ankle axis position in shank segment
% With regards to distal endpoints
...
Tt_implant2bone = [eye(3),[-0.00525; -0.4040 + 0.3960; 0]; [0, 0, 0, 1]];
% From Delp mechanism to mid-epicondyle

Tf_implant2bone = [eye(3),[-0.00525; -0.4040 + 0.3960; 0]; [0, 0, 0, 1]];
% From Delp mechanism to mid-epicondyle
...
% Hinge in femur
rVs65 = ([0.0026; -0.0012; 0.0000] + ... % SCoRE results
    [0.00525; 0.4040 - 0.3960; 0]); % ... % From Delp mechanism to mid-epicondyle
%     *(Segment(5).L/0.4040); % Scaled by thigh length
% Origin at rD5
...
% Hinge in patella
rVs14 = [-0.0377; 0.0318; -0.0000]; % ... % SCoRE results
%     *(Segment(5).L/0.4040); % Scaled by thigh length
% Origin at rD4
...
rVs93 = ([0.039; -0.082; 0.000] + ... % TT
        [0.00525; 0.4040 - 0.3960; 0]); % ... % From Delp mechanism to mid-epicondyle
%     *(Segment(3).L/0.3914); % Scaled by shank length
...
Joint(4).d(1,1) = norm([-0.0377; 0.0318; -0.0000] ... % rP4 to rD5
    *(Segment(5).L/0.4040) ... % Scaled by thigh length
    + [0.039; -0.082; 0.000]) ... % rP3(assumed = rD5) to rV93
    *(Segment(3).L/0.3914); % Scaled by shank length
...
Segment(4).nV(:,1) = [0;-1;0] + inv(Segment(4).B)*rVs14; % V14: virtual marker 1 of segment 4
