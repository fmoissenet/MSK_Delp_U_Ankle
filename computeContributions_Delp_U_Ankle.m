...
for i = 1:length(list)
    ...
    cCoP    = Mprod_array3(1/dot(F,F),cross(F,M))+Mprod_array3(lambda,F);% - ...
             %Mprod_array3(1/(dot(F,Y).*dot(F,F)), ...
              %            Mprod_array3(dot(F,M),cross(F,Y))) + ...                        
              %            Mprod_array3(lambda,F);
    CoPx(:,i) = permute(cCoP(1,:,:) + Segment(1).Q(4,:,1:sw),[3,2,1]);
    CoPy(:,i) = permute(cCoP(2,:,:) + Segment(1).Q(5,:,1:sw),[3,2,1]);
    CoPz(:,i) = permute(cCoP(3,:,:) + Segment(1).Q(6,:,1:sw),[3,2,1]);
end
